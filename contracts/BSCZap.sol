...
contract BSCZap is Ownable, IZap {
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external override {
        ...
        IThugswapRouter02(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);
    }
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external override {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtBNB;
            (amtToken, amtBNB) = IThugswapRouter02(routerAddr).removeLiquidityBNB(token, amount, 0, 0, address(this), block.timestamp);
            ...
            uint amt0;
            uint amt1;
            (amt0, amt1) = IThugswapRouter02(routerAddr).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
    ...
    function _swapHalfNativeAndProvide(address token, uint amount, address routerAddress, address recipient) private returns (uint, uint, uint) {
            ...
            if (useNativeRouter[routerAddress]) {
                IThugswapRouter02 router = IThugswapRouter02(routerAddress);
                return router.addLiquidityBNB{value : amount.sub(swapValue)}(token, tokenAmount, 0, 0, recipient, block.timestamp);
            }
            else {
    ...
    function _swapNativeForToken(address token, uint value, address recipient, address routerAddr) private returns (uint) {
        address[] memory path;
        IThugswapRouter02 router = IThugswapRouter02(routerAddr);
        ...
        uint[] memory amounts = router.swapExactBNBForTokens{value : value}(0, path, recipient, block.timestamp);
    ...
    function _swapTokenForNative(address token, uint amount, address recipient, address routerAddr) private returns (uint) {
        ...
        if (isFeeOnTransfer[token]) {
            router.swapExactTokensForBNBSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp);
            return IERC20(token).balanceOf(address(this));
        } else {
            uint[] memory amounts = router.swapExactTokensForBNB(amount, 0, path, recipient, block.timestamp);
            return amounts[amounts.length - 1];
        }
    }
