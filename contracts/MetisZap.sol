...
import "./interfaces/IHyperMetisRouter02.sol";
import "./interfaces/IUniswapV2Pair.sol";
import "./libraries/ftm/TransferHelper.sol";
import "./interfaces/IZap.sol";

contract MetisZap is Ownable, IZap {
    ...
    function zapInToken(address _from, uint amount, address _to, address routerAddr, address _recipient) public override {
    ...
    function zapIn(address _to, address routerAddr, address _recipient) external payable override {
        zapInToken(WNATIVE, msg.value, _to, routerAddr, _recipient);
    }
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external override {
        ...
        IHyperMetisRouter02(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);
    }

    function zapOutToken(address _from, uint amount, address _to, address routerAddr, address _recipient) public override {
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external override {
        zapOutToken(_from, amount, WNATIVE, routerAddr, _recipient);
    }

    function swapToken(address _from, uint amount, address _to, address routerAddr, address _recipient) public override {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
    ...
    function swapToNative(address _from, uint amount, address routerAddr, address _recipient) external override {
        swapToken(_from, amount, WNATIVE, routerAddr, _recipient);
    }
    ...
    function _swapHalfNativeAndProvide(address token, uint amount, address routerAddress, address recipient) private returns (uint, uint, uint) {
            ...
            if (useNativeRouter[routerAddress]) {
                IHyperMetisRouter02 router = IHyperMetisRouter02(routerAddress);
                return router.addLiquidityMetis{value : amount.sub(swapValue)}(token, tokenAmount, 0, 0, recipient, block.timestamp);
            }
            else {
    ...
    function _swapNativeForToken(address token, uint value, address recipient, address routerAddr) private returns (uint) {
        address[] memory path;
        IHyperMetisRouter02 router = IHyperMetisRouter02(routerAddr);
        ...
        uint[] memory amounts = router.swapExactMetisForTokens{value : value}(0, path, recipient, block.timestamp);
    ...
    function _swapTokenForNative(address token, uint amount, address recipient, address routerAddr) private returns (uint) {
        ...
        if (tokenBridgeForRouter[token][routerAddr] != address(0)) {
            ...
            path[0] = token;
            path[1] = tokenBridgeForRouter[token][routerAddr];
            path[2] = router.WMETIS();
        } else {
            path = new address[](2);
            path[0] = token;
            path[1] = router.WMETIS();
        }

        if (isFeeOnTransfer[token]) {
            router.swapExactTokensForMetisSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp);
            return IERC20(token).balanceOf(address(this));
        } else {
            uint[] memory amounts = router.swapExactTokensForMetis(amount, 0, path, recipient, block.timestamp);
            return amounts[amounts.length - 1];
        }
    }
